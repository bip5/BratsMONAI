...
if __name__=="__main__":
    ...
    val_dataset=BratsDataset("/scratch/a.bip5/BraTS 2021/RSNA_ASNR_MICCAI_BraTS2021_ValidationData",transform=val_transform)
    ...
    for epoch in range(max_epochs): # For a given number of epochs
        ...
        while max_index<train_dataset.__len__():
           # one completion of this =one epoch
                ...
                if metric>best_metric_train:
                    ...
                    if metric_val>best_metric:
                        ...
                        if epoch>0 or steps>50:
                            torch.save(
                                        model1.state_dict(),
                                        os.path.join(root_dir,"TALwVal"+ date.today().isoformat()+'T'+str(datetime.today().hour)+'b'+ str(args.bunch)+"ms"+str(args.max_samples)+"ep"+str(best_metric_epoch)+"s"+str(steps)))
            ...
            ########### STANDARD COMPARISON and adding worst perf
            if metric1>metric2:
                if metric1>metric3:                   
