            ...
            with torch.no_grad():
                ...
                if metric > best_metric:
                    ...
                    if CV_flag==1:
                        ...
                        torch.save(
                            model.state_dict(),
                            os.path.join(root_dir, date.today().isoformat()+'T'+str(total_start)+sheet_name+model_name),
                        )
                        if best_metric>0.5:
                            print('A NEW MASTER WILL BE BORN')
                            break
...
if fs_ensemble == 1:
    ...
    print(" cross val data flag set") # this is printed   
    train_dataset =Subset(full_dataset, train_indices)
    val_dataset = Subset(full_dataset, val_indices)
    trainingfunc(train_dataset, val_dataset)
elif exp_ensemble==1:
    print('Expert Ensemble going ahead now')
    val_indices=np.arange(1)
    train_indices=np.arange(1,9)
    files='/scratch/a.bip5/BraTS 2021/selected_files_seed.xlsx'
    ...
    for sheet in sheet_names:      
        full_dataset = ExpDataset(files,sheet, transform=train_transform)
        train_dataset =Subset(full_dataset, train_indices)
        val_dataset = Subset(full_dataset, val_indices)       
        trainingfunc(train_dataset, val_dataset,sheet_name=sheet)
else:
