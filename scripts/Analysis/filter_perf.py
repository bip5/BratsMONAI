...
source_path='/scratch/a.bip5/BraTS/Itemised Dice'
for filename in os.listdir(source_path):
    individual_scores_path=f'/scratch/a.bip5/BraTS/Itemised Dice/{filename}'
    savename='MFP_'+individual_scores_path.split('/')[-1]
    writer=pd.ExcelWriter(f'{savename}.xlsx',engine='xlsxwriter')
    ...
    for sheet in sheets:
        ...
        merged_df = pd.merge(df1, df2, left_on='Subject ID', right_on='key', how='inner').drop(columns=['subject_id','key','Unnamed: 0','mask_path'])
        ...
        for column in merged_df.columns[8:]:
            cm,pm=pearsonr(merged_df[column],merged_df['SegResNetCV1_j7688198ep128'])
        ...
        for i,cluster in enumerate(merged_df['Cluster'].unique()):
            ...
            relevant_columns = [col for col in cluster_df.columns[3:8] if cluster in col or col == 'SegResNetCV1_j7688198ep128']
            plot_df = cluster_df[relevant_columns]
            remaining_column=plot_df.columns.tolist()
            remaining_column.remove('SegResNetCV1_j7688198ep128')
            
            plot_df['Delta_Performance'] = plot_df[remaining_column[0]]-plot_df['SegResNetCV1_j7688198ep128'] 
            plt.scatter(plot_df['SegResNetCV1_j7688198ep128'], plot_df['Delta_Performance'], label=f'{cluster}') 
            
            for column in cluster_df.columns[8:]:
                              
                c,p=pearsonr(cluster_df[column],cluster_df['SegResNetCV1_j7688198ep128'])
            ...
            cluster_dfs['orig'+sheet+cluster]=cluster_df
            cluster_dfs[sheet+cluster]=pd.DataFrame(c_p,index=columns).sort_values(by=['Correlation'],axis=0,ascending=False)
