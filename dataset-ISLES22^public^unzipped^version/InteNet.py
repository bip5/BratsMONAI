...
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,# Copyright (c) MONAI Consortium
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#     http://www.apache.org/licenses/LICENSE-2.0
...
class CustomConv(nn.Module):
    def __init__(self,  spatial_dims, in_channels, out_channels, kernel_size, padding,conv_pre):
        super().__init__()
        self.conv_pre = conv_pre #pre defined convolution filter for all layers     
...
class InteRNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.convp = Conv[Conv.CONV, spatial_dims](
        ...
        self.convp2 = Conv[Conv.CONV, spatial_dims](
            in_channels=in_channels*2,
            out_channels=in_channels,
            kernel_size=1,
            stride=1,
        ...
        self.custom_conv = CustomConv(
            spatial_dims, in_channels, in_channels, kernel_size,padding=padding, conv_pre=conv_pre
        )

    def forward(self, x):
        identity = x
        x = self.conv1(self.act1(self.norm1(x)))
        x_c = self.custom_conv(self.act1(self.norm1(x)))
        ...
        # Apply custom conv
        x_c = self.custom_conv(x)
        
        
        return x

class InteRNetEncoder(nn.Module):
    def __init__(
        self,
        spatial_dims: int = 3,
        init_filters: int = 32,
        ...
        conv_pre = Conv[Conv.CONV, spatial_dims](in_channels=filters, out_channels=filters,groups=filters, kernel_size=kernel_size, padding=padding, bias=False)
        ...
        for i in range(len(blocks_down)):
            ...
            for _ in range(blocks_down[i]):
                ...
                # Create InteRNetBlock instances with the created conv layers
                block = InteRNetBlock(spatial_dims=spatial_dims, in_channels=filters, kernel_size=kernel_size, norm=norm, act=act, conv_pre=conv_pre )
                blocks.append(block)
...
class InteRNet(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_up):
            ...
            blocks = [
                InteRNetBlock(spatial_dims=spatial_dims, in_channels=filters, kernel_size=kernel_size, norm=norm, act=act,conv_pre=conv_pre)
    ...
    def __init__(
        ...
        for i in range(len(blocks_down)):
            ...
            for _ in range(blocks_down[i]):
                ...
                # Create SegResBlock instances with the created conv layers
                block = SegResBlock(spatial_dims=spatial_dims, in_channels=filters, kernel_size=kernel_size, norm=norm, act=act, conv_lower=conv_lower, conv_upper=conv_upper)
                blocks.append(block)
...
class SegResNetDS(nn.Module):
    ...
    def __init__(
        ...
        filters = init_filters 
        self.up_layers = nn.ModuleList()
        conv_lower = Conv[Conv.CONV, spatial_dims](in_channels=filters, out_channels=filters, kernel_size=3, padding=1, bias=False)
        conv_upper = Conv[Conv.CONV, spatial_dims](in_channels=filters, out_channels=filters, kernel_size=3, padding=1, bias=False)
        for i in range(n_up):
            ...
            blocks = [
                SegResBlock(spatial_dims=spatial_dims, in_channels=filters, kernel_size=kernel_size, norm=norm, act=act,conv_upper=conv_upper,conv_lower=conv_lower)
                for _ in range(blocks_up[i])
            ]
            level["blocks"] = nn.Sequential(*blocks)
