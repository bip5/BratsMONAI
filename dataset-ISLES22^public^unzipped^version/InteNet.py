...
class CustomConv(nn.Module):
    def __init__(self,  spatial_dims, in_channels, out_channels, kernel_size, padding,conv_lower, conv_upper,):
        super().__init__()
        self.conv_lower = conv_lower
        self.conv_upper = conv_upper
        self.switch = nn.Parameter(torch.randn(1))
    ...
    def forward(self, x):
        lower_output = self.conv_lower(x)
        upper_output = self.conv_upper(x)

        # Apply sigmoid activation to the switch parameter
        switch_sigmoid = torch.sigmoid(self.switch)

        output = switch_sigmoid * lower_output + (1 - switch_sigmoid) * upper_output
        return output
...
class SegResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv3 = Conv[Conv.CONV, spatial_dims](
            in_channels=in_channels,
            out_channels=in_channels,
            kernel_size=1,
            stride=1,
        ...
        self.custom_conv = CustomConv(
            spatial_dims, in_channels, in_channels, kernel_size,padding=padding, conv_lower=conv_lower, conv_upper=conv_upper
        )
...
class SegResEncoder(nn.Module):
    def __init__(
        ...
        conv_lower = Conv[Conv.CONV, spatial_dims](in_channels=filters, out_channels=filters, kernel_size=kernel_size, padding=padding, bias=False)
        conv_upper = Conv[Conv.CONV, spatial_dims](in_channels=filters, out_channels=filters, kernel_size=kernel_size, padding=padding, bias=False)
